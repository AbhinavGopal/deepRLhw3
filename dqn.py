...
class QLearner(object):
  ...
  def __init__(
    ...
    q_prediction = q_func(obs_t_float, self.num_actions, "q_func", False)
    self.action_prediction = tf.argmax(q_prediction, axis = 1)
    prediction_q= tf.reduce_sum(q_prediction*tf.one_hot(self.act_t_ph, self.num_actions,1.0, 0.0), reduction_indices = -1, name = 'target_q')

    target_prediction = q_func(obs_tp1_float, self.num_actions, "target_q", False)
    max_q_target = self.rew_t_ph + (1-self.done_mask_ph) * gamma * tf.reduce_max(target_prediction)

    self.total_error = huber_loss(prediction_q - tf.stop_gradient(max_q_target))
    #value of delta is automatically 0
  ...
  def step_env(self):
    ...
    if (not self.model_initialized) or random.random() < self.exploration.value(self.t):
      ...
      action = self.session.run(self.action_prediction, {self.obs_t_ph: [observations]})[0]
      self.obs, self.reward, self.done, self.info = self.env.step(action)
